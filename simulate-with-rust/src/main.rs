use ldpc::{noise::Probability, codes::LinearCode, decoders::LinearDecoder};
use rand::thread_rng;
use anyhow::Result;
...
fn main() -> Result<()> {
    ...
    let decoder = ldpc::decoders::BpDecoder::new(code.parity_check_matrix(), probability, num_iterations);

    let re = decoder.decode(received_message.as_view());

    println!("original_message: {original_message:#?}, error: {error:#?}, received_message: {received_message:#?}, re: {re:#?}");
